...
<?php
	if($_REQUEST["GT"] == "1")
	{
		echo("<h3 style='text-align:center;'>TOP SẢN PHẨM PUMA NAM</h3>");
	}
	else
	{
		echo("<h3 style='text-align:center;'>TOP SẢN PHẨM PUMA NỮ</h3>");
	}
?>
    ...
    <div class="row">
		...
		{
			$sql="select DISTINCT sp.MaSanPham, sp.TenSanPham,sp.Gia, sp.GioiTinh,sp.MaDanhMuc,sp.HinhAnh1 from sanpham sp, kichthuoc_sanpham ks where sp.MaSanPham=ks.MaSanPham AND ks.SoLuong>0 AND MaDanhMuc='3' AND GioiTinh='".$_REQUEST["GT"]."' ORDER BY MaSanPham ASC";
